...
#[Instructions(
    ...
    'Catálogo: listar-modelos (tenant_id) → platform, status active|inactive, available_until, days_until_expiration, availability, pricing. Filtrar generations con model= el id/slug del catálogo (no la etiqueta display_name). by_model[] trae model_id, display_name, count. model= también acepta la etiqueta o mayúsculas/-/_ ; agrupa al mismo slug. '.
    ...
    'get_account_generations / generaciones-usuario: ítems nuevos pueden traer usage_record_id y cost_usd (T1; histórico pre-migración null sin backfill). Costo de UNA pieza → cost_usd del ítem; uso-cuenta-tenant es agregado. Ítem.model y by_model[].model_id son slug canónico; display_name es la etiqueta. Filtros opcionales model, platform, status. Fallidos (status=failed) visibles post-T7. '.
    ...
    'Costos: uso-cuenta-tenant by_model[] prefiere platform de usage_metrics (T5+, registros nuevos); si no, proveedor del catálogo. integrity{} compara historial creativo vs usage (imagen/video/prompt) a nivel cuenta y fechas; no cambia con page de by-user. Chat y presentaciones no entran. '.
