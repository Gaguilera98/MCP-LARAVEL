...
#[Name('obtener-envio-formulario-tenant')]
#[Description('Obtiene el detalle de un envío (submission) concreto de un formulario de un tenant.')]
class ObtenerEnvioFormularioTenant extends Tool
{
    public function handle(Request $request): Response|ResponseFactory
    {
        try {
            ...
            $url = config('services.zalo_api.base_url')
                .'/api/v1/tenants/'.$tenantId.'/forms/'.$formId.'/submissions/'.$submissionId;
    ...
    public function schema(JsonSchema $schema): array
    {
        return [
            ...
            'submission_id' => $schema->string()
                ->description('Id del envío (submission).')
                ->required(),
        ];
    }
}
